...
the association travels, which engagement components matter most, whether engagement
affects all students equally, and how practitioners should use the signal safely.
Cumulative engagement remains moderately associated with final grade when cohorts
are pooled ($r=0.32$). \textbf{Component decomposition} (LMG relative-importance
analysis) shows that Diversity and Immediacy account for more unique explained
variance than Frequency (pooled shares at week~11: 38\%, 36\% and 27\%
respectively), even though all three correlate similarly with grade in bivariate
analysis---breadth and promptness of resource use outweigh raw click volume.
\textbf{Quantile regression} shows that the engagement--grade slope is steeper
among lower-performing students than at the median in every cohort (mean excess
at $\tau=0.1$ versus $\tau=0.5$: $+2.3$ grade points per standard deviation of
engagement), supporting targeted support for at-risk learners rather than a
uniform predictor across the ability distribution. Univariate quantile models
identify Immediacy as the component with the strongest lower-tail association.
Trajectory classes and early-warning precision are reported as supporting
diagnostics. Delivery-period comparisons are descriptive because delivery mode is
confounded with cohort and academic year. The metric suits low-cost supportive
...
validation and interpretive extension. Two questions receive particular emphasis.
\textbf{First}, which aspects of engagement---Frequency, Immediacy and
Diversity---are most strongly associated with academic performance once their
overlap is accounted for? Relative-importance (LMG) decomposition addresses this
under the high collinearity among the three indicators. \textbf{Second}, does
engagement affect all students equally, or is the association stronger among
lower-performing students? Quantile regression examines the engagement--grade
relationship across the grade distribution rather than at the average alone.
Additional analyses pool evidence across cohorts, test temporal regularity and
engagement trajectories, and examine robustness and practical limits on student
support.

The report is organised around these priorities. Section~4.2 develops the
component-importance question in detail; Section~4.3 develops the quantile-regression
question. Broader validation (pooled association, quintile separation), trajectory
diagnostics, early-warning precision and robustness checks provide supporting
context. Delivery-period comparisons are treated as descriptive boundary checks
rather than causal tests.
...
Two substantive questions guide the analysis reported in Sections~4.2 and~4.3.
\textbf{RQ1 (component importance):} Which of Frequency, Immediacy and Diversity
contributes most strongly to explaining variation in final grades? Because the
three indicators are highly correlated (mean pairwise $|r| \approx 0.75$--$0.83$;
VIF $\approx 2.6$--$4.4$), ordinary partial regression coefficients are unstable.
The LMG relative-importance decomposition \citep{lindeman1980} partitions model
$R^2$ into fair shares that are stable under collinearity, computed per cohort at
weeks~6 (mid-term) and~11 (end-of-term). Bivariate rank correlations from
meta-analysis provide a contrast: they show whether each indicator associates with
grade in isolation, without removing shared variance.

\textbf{RQ2 (heterogeneity across the grade distribution):} Does engagement affect
all students equally? Quantile regression \citep{koenker1978} estimates the
relationship between standardised cumulative engagement and final grade at
selected quantiles $\tau \in \{0.1, 0.25, 0.5, 0.75, 0.9\}$, interpreted as the
grade-point change per one standard deviation increase in engagement among students
at the $\tau$-quantile of the grade distribution. The contrast between $\tau=0.1$
(lower tail) and $\tau=0.5$ (median) is summarised descriptively and with
student-level bootstrap confidence intervals (500 replicates per cohort). Univariate
quantile models (one indicator at a time) identify which component drives
lower-tail associations. Analyses use weeks~6 and~11.
...
\section{Results}
The results open with overall engagement--grade association (Section~4.1), then
address the two primary research questions in detail: component importance
(Section~4.2) and heterogeneity across the grade distribution (Section~4.3).
Supporting diagnostics, practical limits and robustness follow.
...
\subsection{RQ1: Which engagement components matter most?}
...
\subsubsection{Method and bivariate contrast}
The LMG decomposition averages each predictor's contribution to $R^2$ over all
orderings in which Frequency, Immediacy and Diversity enter the model, yielding
a stable relative-importance share per indicator \citep{lindeman1980}. As a
contrast, bivariate pooled rank correlations at week~11 show similar associations
...
\subsubsection{Results at week 11}
...
\subsubsection{Stability at mid-term (week 6)}
...
In the engagement literature, behavioural engagement is not reducible to
time-on-task: it reflects participation breadth, effort and timely involvement
with academic work \citep{fredricks2004}. Frequency counts log events and is
sensitive to superficial or repeated access without sustained study. Diversity
captures whether students explore the full range of chapter resources---curriculum
coverage rather than revisiting a single file. Immediacy reflects alignment with
the release schedule, consistent with staying current rather than falling behind.
The LMG results indicate that outcome relevance lies in indicators of structured
study behaviour, not raw click volume. \textbf{Takeaway:} engagement matters, but
breadth and promptness carry more unique signal than session frequency; bivariate
correlations alone would not reveal this distinction.

\subsection{RQ2: Does engagement affect all students equally?}
...
Models take the form $\mathrm{grade} = \beta_0 + \beta_1(\tau)\, z_{\mathrm{IDF}}
+ \varepsilon$, where $z_{\mathrm{IDF}}$ is within-cohort standardised cumulative
engagement and $\beta_1(\tau)$ is the slope at quantile $\tau$ of the grade
distribution. Positive $\beta_1(\tau)$ means that higher engagement is associated
with higher grades among students at that performance level. If engagement is a
\textbf{support tool for at-risk learners}, slopes should be largest at low
$\tau$; if it is a \textbf{universal predictor}, slopes should be similar across
$\tau$.

\subsubsection{Results: composite engagement (IDF)}
...
\subsubsection{Which component drives lower-tail effects?}
...
\subsubsection{Interpretation}
The quantile results support viewing engagement as a \textbf{support tool for
at-risk learners} rather than a predictor with uniform effect across the ability
distribution. Interventions that raise engagement among disengaged, lower-performing
students are associated with the largest grade differences; among already
high-performing students, additional engagement variation is less consequential.
\textbf{Takeaway:} engagement variation is most valuable for identifying and
supporting academically at-risk students, not for ranking the entire cohort with
equal precision.

\subsection{Supporting diagnostics: trajectories, temporal patterns and assessment validity}
...
Latent-class analysis of weekly cumulative IDF profiles identifies interpretable
trajectory types---low-steady, high-steady and mid-variable
...
\subsection{Practical limits: early-warning precision}
Although quantile regression shows greater outcome relevance in the lower grade
tail (Section~4.3), operational flagging still faces modest precision. At the
...
\section{Discussion}
The results support three linked conclusions, with the first two corresponding to
the primary research questions developed in Sections~4.2 and~4.3.

\textbf{RQ1 (component importance).} Once collinearity among Frequency, Immediacy
and Diversity is accounted for, Diversity and Immediacy carry more unique
information than Frequency (mean LMG shares at week~11: 37\%, 36\% and 27\%).
The ranking is broadly stable at week~6, when practitioners might first act on
engagement data. Bivariate correlations would misleadingly suggest all three
matter equally ($r \approx 0.28$--$0.31$); only decomposition reveals that raw
click volume contributes least. This aligns with \citet{fredricks2004}'s
distinction between behavioural engagement as structured participation and mere
time-on-task.

\textbf{RQ2 (heterogeneity across the grade distribution).} Quantile regression
shows that engagement--grade associations are strongest among lower-performing
students: in every cohort the slope at $\tau=0.1$ exceeds that at the median,
with a mean excess of 2.3 grade points per standard deviation of engagement.
Engagement is therefore best understood as a \emph{support tool for at-risk
learners} rather than a uniform predictor across ability levels. Univariate
quantile models suggest Immediacy is especially salient in the lower tail when
indicators are considered separately, complementing the multivariate LMG finding
that Diversity carries the most unique variance overall.

\textbf{External validation.} The chapter-aligned metric remains a moderate,
portable engagement signal in new STAT0002 and STAT0004 cohorts: pooled
$r=0.32$, a mixed-effects slope of roughly 3.8 grade points per standard
deviation, positive associations in every cohort, and clear quintile separation
in grade outcomes. This extends the primary findings of \citet{johnston2025}
through pooled inference across module-year contexts not used in the original
validation study.

\textbf{Supporting diagnostics.} Sustained engagement trajectories separate
final grades by roughly 9.7 points between low-steady and high-steady profiles;
partial weekly profiles recover the low-steady class with about 81\% recall by
week~4. Temporal regularity adds explanatory power in STAT0002 but not STAT0004,
and engagement aligns more closely with individually assessed work than with
peer- or group-marked components.

\textbf{Practical limits.} The association is concentrated in the lower grade
tail, but bottom-quintile precision is modest (about 15--26\%). The metric
identifies students with low \emph{observed} Moodle engagement, not students who
will fail. Delivery-period comparisons provide no evidence of a large moderation
effect by delivery label, but because condition is perfectly confounded with
academic year and cohort, they cannot establish that delivery mode is irrelevant.
